# Supabase Campus Chat Setup

Run this SQL in the Supabase SQL Editor before using the Chat tab.

## What this creates

- `chat_posts` — anonymous venue-tagged messages (campus-wide feed)
- `chat_votes` — one vote per user per post (`+1` / `-1`), toggleable off
- `chat_reports` — user reports
- RPCs for create / vote / soft-delete / report with server checks
- Soft-hide at score ≤ −4; author soft-delete also sets `is_hidden`
- 24h expiry (`expires_at`) + cleanup helper
- Realtime on `chat_posts`

## SQL

Paste the contents of [`supabase/chat_setup.sql`](./supabase/chat_setup.sql) into the Supabase SQL Editor, or copy the block below.

```sql
-- =============================================================================
-- Campus Chat (Yik Yak–style) for Bar Fest
-- =============================================================================
...
> If `ALTER PUBLICATION ... ADD TABLE chat_posts` errors because the table is already in the publication, you can ignore that line.

## Optional: scheduled 24h cleanup

Feeds already filter `expires_at > now()`, so expired posts disappear from the app immediately. To hard-delete them, schedule `cleanup_expired_chat_posts()`:

### Option A — Supabase Dashboard cron (pg_cron)

```sql
-- Requires pg_cron enabled (Database → Extensions)
SELECT cron.schedule(
  'cleanup-expired-chat-posts',
  '15 * * * *', -- every hour at :15
  $$SELECT cleanup_expired_chat_posts();$$
);
```
...
| Author delete | `hide_own_chat_post` → `is_hidden` |
| 24h lifetime | `expires_at` + cleanup function |

## App wiring

The Chat tab calls these RPCs via `src/features/chat/chatService.ts`. Client rechecks GPS and upserts `live_locations` before `create_chat_post`.
