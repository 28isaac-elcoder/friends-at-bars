// MapKit JS 5 — same pattern as the main Bar Fest app
  ...
  loadPromise = new Promise<void>((resolve, reject) => {
    ...
    if (existing) {
      existing.addEventListener("load", () =>
        initMapKit().then(resolve).catch(reject)
      );
...
function initMapKit(): Promise<void> {
  ...
  if (!token) {
    return Promise.reject(new Error("VITE_MAPKIT_TOKEN is not set"));
  }
  return new Promise((resolve, reject) => {
    try {
      ...
      if (e instanceof Error && /already initialized/i.test(e.message)) {
        mapKitInitialized = true;
        resolve();
        return;
      }
