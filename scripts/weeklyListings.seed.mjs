/**
 * Mirrors src/data/dealsAndEvents.ts WEEKLY_LISTINGS for catalog seed.
 * Keep in sync when web deals change.
 */
...
export function listingKind(typeLabels) {
  const hasE = typeLabels.includes("Event");
  const hasD = typeLabels.includes("Drink Special");
  const hasF = typeLabels.includes("Food Special");
